...
const processJob = async (job) => {
    ...
    try {
       ...
       WHERE id = $1`, [uploadId]);
            ...
            const upload = rows[0];
            // Mark as parsing
            await client.query(`UPDATE public.uploads
       ...
       WHERE id = $1`, [uploadId]);
            ...
            const statementInsert = await client.query(`INSERT INTO public.statements (upload_id, bank_name, currency, raw_metadata, created_at, updated_at)
       ...
       RETURNING id`, [
            ...
            for (const raw of rawRows) {
                let tx;
                try {
                    tx = (0, normalizeTransaction_1.normalizeTransaction)(raw, bankConfig);
                }
                catch (normErr) {
                    skippedRows++;
                    logger_1.logger.warn({ err: normErr.message, rawDate: raw.rawDate, description: raw.rawDescription }, "Skipping row that failed normalization");
                    continue;
                }
                ...
                await client.query(`INSERT INTO public.transactions
         ...
         VALUES ($1, $2, $3, $4, $5,
            ...
            if (skippedRows > 0) {
                logger_1.logger.warn({ uploadId, skippedRows }, "Some rows were skipped during normalization");
            }
            if (minDate && maxDate) {
                await client.query(`UPDATE public.statements
       ...
       WHERE id = $1`, [uploadId]);
        ...
        await (0, pool_1.withClient)(async (client) => {
         ...
         SET status = 'failed',
             error = $2,
             updated_at = now()
         WHERE id = $1`, [uploadId, err?.message ?? "Parse upload failed"]);
        });
        logger_1.logger.error({ err, uploadId, jobId: job.id }, "parse-upload job failed");
        throw err;
    }
};
...
exports.parseUploadWorker.on("ready", () => {
    logger_1.logger.info("parse-upload worker ready");
});
exports.parseUploadWorker.on("active", (job) => {
    logger_1.logger.info({ jobId: job.id, uploadId: job.data?.uploadId }, "parse-upload job active");
});
exports.parseUploadWorker.on("completed", (job) => {
    logger_1.logger.info({ jobId: job.id, uploadId: job.data?.uploadId }, "parse-upload job completed");
});
exports.parseUploadWorker.on("failed", (job, err) => {
    logger_1.logger.error({ jobId: job?.id, uploadId: job?.data?.uploadId, err }, "parse-upload job failed");
});
exports.parseUploadWorker.on("error", (err) => {
    logger_1.logger.error({ err }, "parse-upload worker error");
});
