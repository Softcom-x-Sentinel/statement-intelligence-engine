...
import { BankConfig, BankPdfConfig } from "@ingestion/types";
...
const createPdfLineParser = (config: BankPdfConfig, bankName: string) => {
  if (
    !config.lineRegex ||
    ...
    throw new Error(
      "BankPdfConfig is missing required line-mode fields: lineRegex, dateGroupIndex, descriptionGroupIndex, amountGroupIndex"
    );
  }

  return (line: string): RawTransactionRow | null => {
...
const parsePdfBlocks = (
  text: string,
  config: BankPdfConfig,
  bankName: string
): RawTransactionRow[] => {
  ...
  if (!blockStartRegex || !amountsTailRegex) {
    throw new Error(
      "BankPdfConfig is missing required block-mode fields: blockStartRegex, amountsTailRegex"
    );
  }

  const blocks: Array<{ postedDate: string; valueDate: string; body: string }> = [];
  let current: { postedDate: string; valueDate: string; body: string } | null = null;

  for (const line of text.split("\n")) {
    const trimmed = line.trim();
    if (!trimmed) continue;
    if (blockSkipRegex?.test(trimmed)) continue;
...
const extractExpectedClosingBalance = (text: string): number | null => {
...
const lastParsedBalance = (rows: RawTransactionRow[]): number | null => {
...
const extractWithClaude = async (
  ...
  logger.info(
    { bankName: bankConfig.bankName, chunkCount: chunks.length },
    "Falling back to Claude PDF extraction"
  );
  ...
  return toLLMRawRows(allTransactions, bankConfig, "pdf");
};
...
export async function parsePdfFile(options: PdfParseOptions): Promise<RawTransactionRow[]> {
  ...
  logger.debug({ bankName: bankConfig.bankName }, "PDF text extracted");
  ...
  try {
    ...
    logger.warn({ err, bankName: bankConfig.bankName }, "Deterministic PDF parse failed, trying Claude");
    return extractWithClaude(text, bankConfig);
  }

  if (rows.length === 0) {
    logger.warn({ bankName: bankConfig.bankName }, "Deterministic parse returned 0 rows, trying Claude");
    return extractWithClaude(text, bankConfig);
  }
  ...
  if (expectedBalance !== null) {
    ...
    if (actualBalance !== null && Math.abs(actualBalance - expectedBalance) > 1) {
      logger.warn(
        { actualBalance, expectedBalance, bankName: bankConfig.bankName },
        "Closing balance mismatch, trying Claude"
      );
      return extractWithClaude(text, bankConfig);
    }
  }
